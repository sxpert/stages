...
function stc_default_menu ($options=null) {
	...
	*/ 
	if ($logged) { 
		...
		stc_menu_add_item ($menu, 'Mon compte', 'account-details.php');
		stc_menu_add_item ($menu, 'Changer mon mot de passe', 'account-password.php');
...
";
  stc_send_email($email, "[stages M2R] Validez votre compte", $message);
}
...
L'administrateur du site
";
	stc_send_email($email, "[stages M2R] Perte de mot de passe", $message);
}
